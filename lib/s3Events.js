...
import { listEventMetadata } from './adminEvents.js';
import { getS3Client, S3_BUCKET, S3_REGION } from './awsS3.js';
...
export async function listEventFolders() {
    ...
    for (const prefix of prefixes) {
        ...
        const childListing = await client.send(new ListObjectsV2Command({
            Bucket: S3_BUCKET,
            Prefix: folder,
        }));
        const assets = childListing.Contents?.filter((item) => item.Key && !item.Key.endsWith('/')).map((item) => ({
            key: item.Key,
    ...
    for (const meta of metadata) {
        if (result.some((event) => event.id === meta.id))
            continue;
        result.push({
            id: meta.id,
            folder: meta.folder,
