...
type DefaultHomeContent = typeof pageDefaults.home;
...
type LanguageContent<T> = Omit<T, 'sponsors'> & {
  sponsors: {
    heading: string;
    subtitle: string;
...
type HomeContent = {
  en: LanguageContent<DefaultHomeContent['en']>;
  es: LanguageContent<DefaultHomeContent['es']>;
};
...
function mergeHomeContent(existing: Record<string, unknown> | undefined): HomeContent {
  const base = deepClone(pageDefaults.home) as unknown as HomeContent;
  ...
  const assignLanguage = (language: 'en' | 'es') => {
    ...
    if (sponsorsSection && typeof sponsorsSection === 'object') {
      const sponsorsObj = sponsorsSection as Record<string, unknown>;
      const sponsorsArray = Array.isArray(sponsorsObj.sponsors)
        ? (sponsorsObj.sponsors as unknown[]).map((sponsor) => deepClone(sponsor) as StoredSponsor)
        : undefined;
      if (sponsorsArray) {
...
function toContentRecord(content: HomeContent): Record<string, unknown> {
  return deepClone(content) as Record<string, unknown>;
}

function normalizeSponsors(content: HomeContent): {
  ...
  const sanitize = (entries: unknown[]): SponsorRecord[] => {
  ...
  const records: SponsorRecord[] = order.map((id) => {
...
async function loadNormalizedHomeContent(): Promise<{
  records: SponsorRecord[];
  content: HomeContent;
  changed: boolean;
}> {
  const page = await getPageContent('home');
  const merged = mergeHomeContent(page?.content as Record<string, unknown> | undefined);
...
function sanitizePayload(payload: SponsorPayload): SponsorPayload {
  return {
...
function applySponsorRecords(content: HomeContent, records: SponsorRecord[]) {
  const sponsorsArray = records.map((record) => ({
    id: record.id,
    name: record.name,
    imageUrl: record.imageUrl,
...
export async function createSponsor(payload: SponsorPayload): Promise<SponsorRecord> {
  if (!payload.imageUrl) {
...
export async function updateSponsor(id: string, payload: SponsorPayload): Promise<SponsorRecord> {
  const normalized = await loadNormalizedHomeContent();
  const records = normalized.records;
  const content = normalized.content;
